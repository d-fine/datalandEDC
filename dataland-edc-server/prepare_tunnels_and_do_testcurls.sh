...
health_response=$(curl -X GET "${dataland_edc_server_uri}:${dataland_edc_server_web_http_port}/api/health" -H "accept: application/json")
...
dataId=$(curl -X POST "${dataland_edc_server_uri}:${dataland_edc_server_web_http_port}/api/dataland/data" -H "accept: application/json" -H "Content-Type: application/json" -d "$test_data")
echo "Received response from post request: $dataId"

echo "Retrieving test data."
get_response=$(curl -X GET "${dataland_edc_server_uri}:${dataland_edc_server_web_http_port}/api/dataland/data/$dataId" -H "accept: application/json")
